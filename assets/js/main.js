		...
		window.addEventListener('scroll', () => {
			...
			if (window.innerWidth > 768) {
				...
				document.querySelectorAll('.scroll-section').forEach((el, i) => {
					if (el.offsetTop - document.querySelector('.header-sticky .header-menu .current_item').clientHeight <= scrollDistance) {
						document.querySelectorAll('.header-sticky .header-menu .current_item').forEach((el) => {
							if (el.classList.contains('active')) {
								el.classList.remove('active');
							}
						});
		
						document.querySelectorAll('.header-sticky .header-menu .current_item')[i].classList.add('active');
					}
				});
			}
		});
